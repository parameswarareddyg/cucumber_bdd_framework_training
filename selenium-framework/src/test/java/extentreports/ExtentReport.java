package extentreports;

import java.awt.Desktop;
import java.io.File;
import java.io.IOException;
import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;
import java.util.Objects;

import com.aventstack.extentreports.ExtentReports;
import com.aventstack.extentreports.reporter.ExtentSparkReporter;
import com.aventstack.extentreports.reporter.configuration.Theme;

import constants.FrameworkConstants;

public class ExtentReport {
	private static ExtentReports extentReports;
	private static String reportDateTime;
	
	public static void initExtentReport()
	{
		if(Objects.isNull(extentReports))
		{
			extentReports = new ExtentReports();
			reportDateTime = getDateAndTime();
			ExtentSparkReporter spark = new ExtentSparkReporter(new File(FrameworkConstants.getReportspath()+reportDateTime+".html"));
			spark.config().setDocumentTitle("FlipKartAutomation Test");
			spark.config().setReportName("Automation TestReport");
			spark.config().setTheme(Theme.DARK);
			extentReports.attachReporter(spark);
			
		}
	}
	
	public static void flushExtentReport()
	{
		if(Objects.nonNull(extentReports))
		{
			extentReports.flush();
		}
		
		ExtentReportManager.unload();
		
		try {
			Desktop.getDesktop().browse(new File(FrameworkConstants.getReportspath()+reportDateTime+".html").toURI());
		} catch (IOException e) {
			e.getMessage();
		}
	}
	
	public static void createTest(String testName) {
		ExtentReportManager.setExtentTest(extentReports.createTest(testName));
	}
	
	public static ExtentReports getExtentReport()
	{
		return extentReports;
	}
	
	public static String getDateAndTime()
	{
		LocalDateTime now = LocalDateTime.now();
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd_MMM_yyyy_hh_mm_ss");
		String formatedDate = now.format(formatter);

		return formatedDate;
	}
}
